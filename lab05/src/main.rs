...
fn p1(file_path : &str) -> Result<(), io::Error>
{
    ...
    let mut biggest_age : i8 = 0;
    let mut biggest_age_name : String = String::new();
    let mut smallest_age : i8 = i8::MAX;
    let mut smallest_age_name : String = String::new();

    for line in file.lines()
    {
        let mut person : Student = Student { name: String::new(), phone: String::new(), age: -1 };

        let mut items = line.split(',');

        person.name = items.next().ok_or(io::Error::other("next()"))?.to_string();
        person.phone = items.next().ok_or(io::Error::other("next()"))?.to_string();
        let age_string = items.next().ok_or(io::Error::other("next()"))?;
        person.age = age_string.parse::<i8>().map_err(io::Error::other)?;

        if person.age > biggest_age
        {
            biggest_age = person.age;
            biggest_age_name = person.name.clone();
        }

        if person.age < smallest_age
        {
            smallest_age = person.age;
            smallest_age_name = person.name.clone();
        }
    }

    println!("Oldest student is {} being {} years old!\nYoungest student is {} being {} years old!", biggest_age_name, biggest_age, smallest_age_name, smallest_age);
    Ok(())
}
...
{
    let c : Canvas = Canvas { canvas: [[' '; 100]; 55] };

    c
}

fn set_pixels(c : &mut Canvas, list : &[(usize, usize, u8)])
{
    for &(x, y, ascii_value) in list
    {
        c.canvas[x][y] = ascii_value as char;
    }
}
...
fn p3(file_path : &str) -> Result<(), io::Error>
{
    let file : String = fs::read_to_string(file_path)?;
    let mut biggest_age : i8 = 0;
    let mut biggest_age_name : String = String::new();
    let mut smallest_age : i8 = i8::MAX;
    let mut smallest_age_name : String = String::new();
    let persons : [Student; 4] = serde_json::from_str(&file)?;
...
fn main()
{
    match p1("student.txt")
    {
        Ok(()) => (),
        Err(e) => println!("{}", e)
    }
    p2();
    match p3("student.json")
    {
        Ok(()) => (),
        Err(e) => println!("{}", e)
    }
